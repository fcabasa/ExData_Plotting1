...
epc_plot2$DateTime <- as.POSIXct(paste(epc_plot2$Date, epc_plot2$Time), format="%Y-%m-%d %H:%M:%S") 

## 5) Create final plot and output png file
png("plot2.png", width=480, height=480)
plot(epc_plot2$DateTime, epc_plot2$Global_active_power, 
     xlab="", ylab="Global Active Power (Kilowatts)", type="n") 
lines(epc_plot2$DateTime, epc_plot2$Global_active_power)
dev.off()
